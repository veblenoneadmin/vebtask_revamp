...
// ── Mailer setup (reuse your existing nodemailer config) ──────────────────────
...
// ── Fetch KPI data (mirrors your /api/kpi-report logic) ──────────────────────
async function getKPIData(orgId, period) {
  ...
  // Fetch org members (owners only for recipients, but all for KPI data)
  ...
  const totalHours = timeEntries.reduce((sum, e) => {
    const hrs = e.duration ? e.duration / 3600 : 0;
    return sum + hrs;
  }, 0);
  ...
  const billableHours  = timeEntries.filter(e => e.billable).reduce((sum, e) => sum + (e.duration ? e.duration / 3600 : 0), 0);
  const billableRatio  = totalHours > 0 ? Math.round((billableHours / totalHours) * 100) : 0;

  // Per-member stats
  const memberStats = members.map(m => {
    const userEntries = timeEntries.filter(e => e.userId === m.userId);
    const userTasks   = tasks.filter(t => t.assigneeId === m.userId);
    const hours       = parseFloat(userEntries.reduce((s, e) => s + (e.duration ? e.duration / 3600 : 0), 0).toFixed(1));
...
function buildEmailHTML(orgName, kpiData) {
  const { label, orgKPIs, memberStats } = kpiData;
  const o = orgKPIs;
  ...
  const memberRow = (m, i) => `
    <tr style="border-bottom:1px solid #2d2d2d;">
      ...
      <td style="padding:10px 12px;font-size:12px;color:#909090;text-transform:capitalize;">${m.role}</td>
  ...
  <div style="max-width:640px;margin:0 auto;padding:24px 16px;">
    ...
    <div style="background:#1e1e1e;border:1px solid #3c3c3c;border-radius:12px;padding:28px;margin-bottom:16px;">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:6px;">
        <div style="width:10px;height:10px;border-radius:50%;background:#007acc;"></div>
        <span style="font-size:11px;color:#909090;text-transform:uppercase;letter-spacing:2px;">KPI Intelligence Report</span>
      </div>
    ...
    <div style="background:#1e1e1e;border:1px solid #3c3c3c;border-radius:12px;overflow:hidden;margin-bottom:16px;">
      <div style="padding:16px 16px 12px;border-bottom:1px solid #2d2d2d;">
        <span style="font-size:13px;font-weight:700;color:#f0f0f0;">Team Performance</span>
      </div>
      <table width="100%" cellpadding="0" cellspacing="0">
        ...
        <tbody>
          ${memberStats.map((m, i) => memberRow(m, i)).join('')}
        </tbody>
      </table>
    </div>` : ''}
    ...
    <div style="text-align:center;padding:16px;">
      <p style="margin:0;font-size:11px;color:#555;">
        Sent automatically by VebTask · 5:00 PM AWST<br>
        <span style="color:#333;">You're receiving this because you're an owner of ${orgName}</span>
      </p>
    </div>
...
async function sendAllOrgs(period) {
  const orgs = await prisma.organization.findMany({ where: { active: true } });
  console.log(`[KPI Scheduler] Sending ${period} reports to ${orgs.length} orgs...`);
  await Promise.allSettled(orgs.map(org => sendKPIEmail(org, period)));
}

// ── Cron jobs — 5PM AWST = 09:00 UTC ─────────────────────────────────────────
export function startKPIScheduler() {
  // Daily — every day at 09:00 UTC (5PM AWST)
  cron.schedule('0 9 * * *', () => sendAllOrgs('daily'), { timezone: 'UTC' });

  // Weekly — every Monday at 09:00 UTC (5PM AWST)
  cron.schedule('0 9 * * 1', () => sendAllOrgs('weekly'), { timezone: 'UTC' });

  // Monthly — 1st of every month at 09:00 UTC (5PM AWST)
  cron.schedule('0 9 1 * *', () => sendAllOrgs('monthly'), { timezone: 'UTC' });

  console.log('[KPI Scheduler] ✅ Scheduled — Daily, Weekly & Monthly at 5PM AWST');
}
